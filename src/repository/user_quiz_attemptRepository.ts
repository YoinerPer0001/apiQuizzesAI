...
class UserQuizAttemptRepository {
  ...
  async getByUserId(user_id: string): Promise<User_Quiz_Attempts[]> {
    return await User_Quiz_Attempts.findAll({
      where: { user_id },
      attributes: { exclude: exclideAttributes },
    });
  }

  async getByQuizIdUserId(quiz_id: string, user_id:string): Promise<User_Quiz_Attempts[]> {
      console.log("UID from token:", user_id);
     console.log("Quiz ID from params:", quiz_id);
    return await User_Quiz_Attempts.findAll({
      where: { quiz_id: quiz_id , user_id: user_id},
      attributes: { exclude: ["updatedAt", "quiz_id", "user_id"] }
    });
  }
  ...
  async create(
    data: CreationAttributes<User_Quiz_Attempts>,
    options?: { transaction?: Transaction | null }
  ): Promise<User_Quiz_Attempts> {
    return await User_Quiz_Attempts.create(data, {
      transaction: options?.transaction ?? null,
    });
  }

  async update(
    data: Partial<CreationAttributes<User_Quiz_Attempts>>,
    id: string,
    options?: { transaction?: Transaction | null }
  ): Promise<number> {
    const [affectedRows] = await User_Quiz_Attempts.update(data, {
      where: { attempt_id: id },
      transaction: options?.transaction ?? null,
    });
    return affectedRows;
