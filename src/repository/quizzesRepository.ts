...
class QuizzesRepository {
    ...
    async getAllPublics(
        ...
        const {rows: quizzes, count: total} = await Quizzes.findAndCountAll({
            attributes: { exclude: exludeAttr , include: [[Sequelize.literal(`(SELECT COUNT(*)FROM "user_quiz_attempts" AS attempts WHERE attempts."quiz_id" = "quizzes"."quiz_id")`),"attemptCount"]]},
            limit,
            offset,
            order: [['createdAt', 'DESC']],
            where: whereClause,
